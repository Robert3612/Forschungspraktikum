...
int main(void)
{
    ...
    // answer to question:
    // Memory controller loads 16 * 32 Bytes
    // every thread only uses the first element, does not need the other 15
    // The other 15 elements are STILL loaded from global memory
    // 93% of memory bandwith wasted
}
